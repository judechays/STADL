...
#dta$COWCODE <- ifelse(dta$year > 1991 & dta$country=="German Federal Republic", "255",dta$COWCODE)
#dta$COWCODE <- ifelse(dta$year > 1991 & dta$country=="Yemen (Arab Republic of Yemen)", "679",dta$COWCODE)
#dta$COWCODE <- ifelse(dta$year > 1991 & dta$country_name=="German Federal Republic", "255",dta$COWCODE)
#dta$COWCODE <- ifelse(dta$year > 1991 & dta$country_name=="Yemen (Arab Republic of Yemen)", "679",dta$COWCODE)

#cshp.dat <- cshp.dat[cshp.dat$COWSYEAR <= as.numeric(as.character(y[yl])),]
#cow2$COWSYEAR = cow2$COWSYEAR + (cow2$CNTRY_NAME == "Macedonia")
